...
STORAGE_FILE_NAME = Path(__file__).parent.parent / "HW8/storage.csv"
...
class Repository:
    ...
    def get_storage(self):
        ...
        for row in reader:
            ...
            try:
                row["marks"] = json.loads(row["marks"])
    ...
    def add_student(self, student: dict):
        ...
        student_to_write = {
            ...
            'name': student['name'],
            'marks': json.dumps([
                {"value": int(m), "date": datetime.date.today().isoformat()} for m in student.get("marks", [])
            ], ensure_ascii=False),
    ...
    def add_mark(self, student_id: str, new_marks: list[int]) -> bool:
        ...
        for student in self.students:
            if student["id"] == student_id:
                student["marks"].extend([
                    {"value": int(mark), "date": datetime.date.today().isoformat()}
            ...
            with open(STORAGE_FILE_NAME, "w", newline='') as file:
                ...
                for s in self.students:
                    writer.writerow({
                        "id": s["id"],
                        "name": s["name"],
                        "marks": json.dumps(s["marks"], ensure_ascii=False),
                        "info": s.get("info", ""),
                    })
            ...
            with open(STORAGE_FILE_NAME, "w", newline='') as file:
                ...
                for student in self.students:
                    writer.writerow({
                        ...
                        "marks": json.dumps(student.get("marks", []), ensure_ascii=False),
                        "info": student.get("info", ""),
                    })
...
# Send report
def total_number_of_students_send():
    with open(STORAGE_FILE_NAME, newline="") as file:
        file.seek(0)
    ...
    msg = EmailMessage()
    msg.set_content(f"Total number of students: {count_students}")

    msg['Subject'] = "Digital Journal Application REPORT regarding students"
    msg['From'] = "journal@application.com"
    msg['To'] = "journal@admin.com"

    s = smtplib.SMTP('localhost')
    s.send_message(msg)
    s.quit()

def send_report_students():
    while True:
        today = datetime.date.today()

        if today.day == 1:
            total_number_of_students_send()

        sleep(60*60*24)

def calculation_average_mark():
    today = datetime.date.today()
    ...
    for student in students:
        for mark in student["marks"]:
            try:
                mark_date = datetime.datetime.strptime(mark["date"], "%Y-%m-%d").date()
                if mark_date == today:
                    marks_of_today.append(mark["value"])
    ...
    if marks_of_today:
        average = sum(marks_of_today) / len(marks_of_today)
        msg.set_content(f"Average mark of today: {average}")
    else:
        msg.set_content(f"No marks found for {today}")

    msg['Subject'] = "Digital Journal Application REPORT regarding an average mark"
    msg['From'] = "journal@application.com"
    msg['To'] = "journal@admin.com"
...
def send_report_average_mark():
    while True:
        calculation_average_mark()
...
if __name__ == "__main__":
    ...
    threading.Thread(target=send_report_students, name="MonthlyReportThread").start()
    threading.Thread(target=send_report_average_mark, name="DailyReportThread").start()