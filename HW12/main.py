...
class DeliveryService(abc.ABC):
    ...
    def _ship(self, delay: float):
        ...
        def _callback():
            ...
            time.sleep(10)
            scheduler.archive_order(self._order)
...
class Scheduler:
    def __init__(self):
        ...
        self.archived_orders: queue.Queue[OrderRequestBody] = queue.Queue()
    ...
    def archive_order(self, order: OrderRequestBody) -> None:
        self.archived_orders.put(order)
        print(f"\n\t{order.order_name} ADDED AT THE ARCHIVE")
